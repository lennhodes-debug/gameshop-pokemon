...
export default function BootSequence() {
  const [phase, setPhase] = useState<'idle' | 'logo' | 'text' | 'burst' | 'done'>('idle');
  const [skip, setSkip] = useState(false);
  const canvasRef = useRef<HTMLCanvasElement>(null);

  useEffect(() => {
    // Alleen bij eerste bezoek van de sessie
  ...
  return (
    <AnimatePresence>
      {(
        <motion.div
          className="fixed inset-0 z-[10000] flex items-center justify-center bg-[#050810] cursor-pointer"
          initial={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        ...
        >
          ...
          {/* SVG Logo — controller icon met stroke animatie */}
