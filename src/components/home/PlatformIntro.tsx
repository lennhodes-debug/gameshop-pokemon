...
const SCENE_MS = 3000;
const TOTAL_SCENES = PLATFORMS.length + 2; // opening + platforms + finale
const MAX_DURATION = SCENE_MS * TOTAL_SCENES + 2000; // safety timeout

export default function PlatformIntro() {
  const [mounted, setMounted] = useState(false);
  const [scene, setScene] = useState(-1); // -1=opening, 0-3=platforms, 4=finale
  ...
  useEffect(() => {
    try {
      if (sessionStorage.getItem('gameshop-intro-v3')) {
        setVisible(false);
        return;
      }
  ...
  const finish = useCallback(() => {
    setVisible(false);
    document.body.style.overflow = '';
    try { sessionStorage.setItem('gameshop-intro-v3', '1'); } catch {}
    if (safetyRef.current) clearTimeout(safetyRef.current);
  }, []);
        ...
        >
          ...
          <AnimatePresence mode="wait">
            ...
            {isPlatform && platform && (
              <motion.div
                key={`platform-${scene}`}
                className="text-center z-10 w-full max-w-4xl mx-auto px-6"
              ...
              >
                ...
                <div
                  ...
                  style={{
                    background: `radial-gradient(ellipse 70% 50% at 50% 50%, ${platform.accent}12 0%, transparent 70%)`,
                  }}
                />
                ...
                {/* Platform naam */}
                <h2 className="text-5xl sm:text-6xl md:text-8xl font-black text-white leading-[0.95] tracking-tight mb-2">
                  {platform.name}
                </h2>
            ...
            {isFinale && (
              ...
              >
                ...
                <p className="mt-6 text-sm sm:text-base text-white/30 font-medium tracking-wider">
                  De Pokémon specialist van Nederland
                </p>
              </motion.div>
            )}
          </AnimatePresence>
