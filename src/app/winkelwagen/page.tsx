        ...
        {items.length === 0 ? (
          ...
          >
            ...
            <p className="text-slate-500 mb-8 max-w-sm mx-auto">
              Ontdek ons assortiment van originele Nintendo games en consoles
            </p>
            <Link href="/shop">
              <motion.span
              ...
              >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {(recentlyViewed.length > 0 ? recentlyViewed : popularProducts).map((product) => {
                    const colors = PLATFORM_COLORS[product.platform] || {
                      from: 'from-slate-500',
                      to: 'to-slate-700',
                    };
                    return (
                      <Link key={product.sku} href={`/shop/${product.sku}`} className="group">
                        <div
                          className={`aspect-square rounded-xl overflow-hidden mb-2 ${product.image ? 'bg-white' : `bg-gradient-to-br ${colors.from} ${colors.to}`}`}
                        >
                          {product.image ? (
                            <Image
                              src={product.image}
                        ...
                        <p className="text-xs font-semibold text-slate-700 line-clamp-2 group-hover:text-emerald-600 transition-colors">
                          {product.name}
                        </p>
                ...
                {rawSubtotal >= FREE_SHIPPING_THRESHOLD && (
                  ...
                  >
                    ...
                    <span className="text-sm font-medium text-emerald-700">
                      Gratis verzending! Je bestelling wordt gratis verzonden via PostNL.
                    </span>
                  </motion.div>
                )}
                    ...
                    return (
                      ...
                      >
                        ...
                        <Link href={`/shop/${item.product.sku}`} className="flex-shrink-0">
                          <div
                            className={`w-24 h-24 rounded-xl ${img ? 'bg-white' : `bg-gradient-to-br ${colors.from} ${colors.to}`} flex items-center justify-center overflow-hidden relative`}
                          >
                            {img ? (
                              <Image
                                src={img}
                              ...
                              <span className="text-white/30 text-xs font-semibold">{label}</span>
                            )}
                          </div>
                        </Link>
                        ...
                        <div className="flex-1 min-w-0">
                          <Link
                            ...
                            className="font-semibold text-slate-900 text-sm transition-colors line-clamp-1"
                          ...
                          <p className="text-xs text-slate-500 mt-0.5 flex items-center gap-1.5">
                            <span
                              className={`h-1.5 w-1.5 rounded-full bg-gradient-to-r ${colors.from} ${colors.to}`}
                            />
                            {item.product.platform} &middot;{' '}
                            {item.variant === 'cib' ? 'Compleet in doos' : item.product.condition}
                            {item.variant === 'cib' && (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {suggestions.map((product) => {
                        const c = PLATFORM_COLORS[product.platform] || {
                          from: 'from-slate-500',
                          to: 'to-slate-700',
                        };
                        return (
                          <div key={product.sku} className="group">
                            <Link href={`/shop/${product.sku}`}>
                              ...
                              >
                                <div
                                  className={`aspect-square relative ${product.image ? 'bg-slate-50' : `bg-gradient-to-br ${c.from} ${c.to}`}`}
                                >
                                  {product.image ? (
                                    <Image
                                      src={product.image}
                                ...
                                <div className="p-3">
                                  ...
                                  <p className="text-[10px] text-slate-500 mt-0.5 flex items-center gap-1">
                                    <span
                                      className={`h-1.5 w-1.5 rounded-full bg-gradient-to-r ${c.from} ${c.to}`}
                                    />
                                    {product.platform}
                                  </p>
                ...
                <div className="bg-white rounded-2xl border border-slate-100 p-6 sticky top-28 shadow-lg">
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between">
                      <span className="text-slate-500">
                        Subtotaal ({items.reduce((s, i) => s + i.quantity, 0)} items)
                      </span>
                        ...
                        <div>
                          <div className="flex gap-2">
                            ...
                            <button
                              ...
                              className="px-4 py-2 rounded-xl bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-100 text-sm font-medium transition-all"
                            >
                              Toepassen
                            </button>
                          </div>
                  ...
                  <Link href="/afrekenen">
                    <motion.span
                      ...
                      whileTap={{ scale: 0.98 }}
                      className="block w-full mt-6 px-6 py-4 rounded-2xl bg-slate-900 text-white font-medium shadow-lg hover:bg-slate-800 transition-all text-center"
                    >
                      Afrekenen
                    </motion.span>
                  </Link>
                  ...
                  <div className="mt-5 flex items-center justify-center gap-2">
                    {['iDEAL'].map((m) => (
                      <span
                        key={m}
                        className="px-2.5 py-1 rounded-lg bg-slate-50 border border-slate-100 text-[10px] text-slate-500 font-medium"
                      >
                        {m}
                      </span>
                    ))}
