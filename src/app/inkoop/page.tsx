...
import { motion, useInView, useScroll, useTransform, useMotionValue, useSpring, useMotionTemplate, AnimatePresence } from 'framer-motion';
import Image from 'next/image';
import { getAllProducts, type Product } from '@/lib/products';

// ─── REUSABLE ANIMATION COMPONENTS ─────────────────────────

function CharReveal({ text, className = '', delay = 0, stagger = 0.025 }: { text: string; className?: string; delay?: number; stagger?: number }) {
...
export default function InkoopPage() {
  ...
  const [form, setForm] = useState({ naam: '', email: '', telefoon: '', platform: '', beschrijving: '' });
  ...
  const products = getAllProducts().filter(p => p.image).slice(0, 8);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setSubmitted(true);
    setTimeout(() => { window.location.href = generateMailto(); }, 300);
  };
    ...
    <div className="pt-16 lg:pt-20">
      ...
      >
        ...
        {products.slice(0, 6).map((product, i) => {
          const positions = [
            { top: '8%', left: '5%', rotate: -12, size: 'w-20 h-20 lg:w-28 lg:h-28', opacity: 0.15 },
            { top: '15%', right: '8%', rotate: 8, size: 'w-16 h-16 lg:w-24 lg:h-24', opacity: 0.12 },
            { bottom: '25%', left: '8%', rotate: 6, size: 'w-14 h-14 lg:w-20 lg:h-20', opacity: 0.1 },
            { bottom: '12%', right: '12%', rotate: -8, size: 'w-18 h-18 lg:w-24 lg:h-24', opacity: 0.13 },
            { top: '45%', left: '2%', rotate: -4, size: 'w-12 h-12 lg:w-16 lg:h-16', opacity: 0.08 },
            { top: '35%', right: '3%', rotate: 10, size: 'w-14 h-14 lg:w-18 lg:h-18', opacity: 0.1 },
          ];
          ...
          return (
            <motion.div
              ...
              style={{ top: pos.top, left: pos.left, right: pos.right, bottom: pos.bottom, opacity: pos.opacity }}
              animate={{
                y: [0, -15 + i * 3, 0],
            ...
            >
              <Image src={product.image!} alt="" width={120} height={120} className="object-cover w-full h-full rounded-2xl" />
            </motion.div>
          );
        })}
                  ...
                  >
                    <form onSubmit={handleSubmit} className="space-y-5">
                      ...
                      <div className="bg-amber-50/70 rounded-2xl px-5 py-4">
                        <div className="flex items-start gap-3">
                          ...
                          <p className="text-slate-600 text-sm leading-relaxed">
                            <strong className="text-slate-800">Tip:</strong> Na het verzenden opent je e-mail app. Voeg foto&apos;s toe van de voor- en achterkant. Duidelijke foto&apos;s = beter bod!
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2 pt-2">
                        {['Reactie binnen 24 uur', 'Eerlijk bod', 'Betaling binnen 2 dagen'].map((text) => (
                          <span key={text} className="inline-flex items-center gap-1.5 text-xs text-slate-500">
              ...
              <div className="bg-white rounded-2xl p-7 shadow-sm">
                ...
                <div className="space-y-5">
                  {[
                    ...
                    >
                      <div className={`h-10 w-10 rounded-xl bg-gradient-to-br ${item.gradient} flex items-center justify-center text-white flex-shrink-0 shadow-md group-hover:shadow-lg transition-shadow`}>
