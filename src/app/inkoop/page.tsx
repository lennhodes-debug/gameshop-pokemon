...
export default function InkoopPage() {
  ...
  return (
    <div className="pt-16 lg:pt-20">
      {/* Hero */}
      <div className="relative bg-[#050810] py-10 lg:py-14 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <p className="text-slate-400 text-base lg:text-lg max-w-xl mb-4">
          ...
          <motion.div
            ...
            className="mt-5"
          >
            <h2 className="text-white font-bold text-xs uppercase tracking-wider mb-2.5 flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
              {[
                ...
                <div key={item.step} className="flex sm:flex-col items-start sm:items-center sm:text-center gap-2 sm:gap-1.5 bg-white/[0.04] rounded-lg px-3 py-2.5 border border-white/[0.06] hover:bg-white/[0.07] transition-colors">
                  <span className="flex-shrink-0 flex items-center justify-center h-6 w-6 rounded-md bg-gradient-to-r from-emerald-500 to-teal-500 text-white text-xs font-bold">
                    {item.step}
                  </span>
                  <div>
                    <h3 className="text-white font-semibold text-xs">{item.title}</h3>
                    <p className="text-slate-500 text-[11px] leading-tight mt-0.5 hidden sm:block">{item.desc}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
        {/* Info banner */}
        <div className="bg-emerald-50/60 dark:bg-emerald-950/30 rounded-lg px-4 py-3 border border-emerald-100/80 dark:border-emerald-800/50 mb-6">
          <div className="flex items-start gap-2.5">
            <svg className="h-4 w-4 text-emerald-500 dark:text-emerald-400 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <p className="text-slate-600 dark:text-slate-300 text-sm leading-relaxed">
              <span className="font-semibold text-slate-700 dark:text-white">Hoe werken onze inkoopprijzen?</span>{' '}
        ...
        <div className="space-y-3 mb-6">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 outline-none text-sm text-slate-800 dark:text-white placeholder:text-slate-400 transition-all bg-white dark:bg-slate-800"
            />
            {search && (
              <button
                onClick={() => setSearch('')}
          ...
          <div className="flex flex-wrap items-center gap-2">
            <select
              ...
              className="px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 text-sm text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 outline-none transition-all cursor-pointer"
            >
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
          {/* Table header */}
          <div className="hidden sm:grid sm:grid-cols-[40px_1fr_140px_120px_130px] bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 px-5 py-2.5">
            <span></span>
            <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Product</span>
            <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-center">Platform</span>
            <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-center">Marktwaarde</span>
            <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-center">Wij bieden</span>
          </div>
            ...
            <AnimatePresence mode="popLayout">
              {filtered.map((product, idx) => (
                <motion.div
                  ...
                  className={`grid grid-cols-1 sm:grid-cols-[40px_1fr_140px_120px_130px] items-center px-4 sm:px-5 py-2.5 transition-colors duration-150 group cursor-pointer ${
                ...
                >
                  ...
                  <div className="hidden sm:flex items-center">
                    <div className={`h-[18px] w-[18px] rounded border-2 flex items-center justify-center transition-all duration-150 ${selected.has(product.sku) ? 'bg-emerald-500 border-emerald-500 scale-105' : 'border-slate-300 dark:border-slate-600 group-hover:border-emerald-400'}`}>
                      {selected.has(product.sku) && (
                  ...
                  <div className="flex items-center gap-3">
                    <div className="relative h-14 w-14 flex-shrink-0 rounded-lg overflow-hidden bg-slate-50 dark:bg-slate-700/50 border border-slate-200/80 dark:border-slate-600/50">
                      {product.image ? (
                        <Image
                          src={product.image}
                          alt={product.name}
                    ...
                    <div className="min-w-0">
                      <p className="font-semibold text-slate-800 dark:text-white text-sm truncate group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors duration-150">
                        {product.name}
                      </p>
                      <p className="text-[11px] text-slate-400 sm:hidden mt-0.5">{product.platform}</p>
                    </div>
                  </div>

                  {/* Platform */}
                  <div className="hidden sm:flex justify-center">
                    <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-slate-100 dark:bg-slate-700/60 text-slate-500 dark:text-slate-400">
                      {product.platform}
                    </span>
                  </div>
                  ...
                  {/* Trade-in price */}
                  <div className="flex sm:justify-center items-center gap-2 mt-1.5 sm:mt-0">
                    <span className="sm:hidden text-[11px] text-slate-400">Inkoopprijs:</span>
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-md bg-emerald-50 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 font-bold text-sm tabular-nums">
                      &euro;{(product.inkoopPrijs || 0).toFixed(2).replace('.', ',')}
                    </span>
                    <button
                      onClick={(e) => { e.stopPropagation(); toggleSelected(product.sku); }}
                      className={`sm:hidden ml-auto px-3 py-1 rounded-md text-xs font-bold transition-all duration-150 ${selected.has(product.sku) ? 'bg-emerald-500 text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-emerald-100 dark:hover:bg-emerald-900 hover:text-emerald-700 dark:hover:text-emerald-400'}`}
                    >
          ...
          {filtered.length === 0 && (
            <div className="px-6 py-12 text-center">
              ...
              <p className="text-slate-500 dark:text-slate-400 font-medium text-sm">Geen resultaten gevonden</p>
              <p className="text-slate-400 dark:text-slate-500 text-xs mt-1">Probeer een andere zoekterm of neem contact op voor een persoonlijk bod</p>
            </div>
          )}
        </div>

        {/* Bottom CTA */}
        <div className="mt-8 bg-gradient-to-r from-[#0a1628] to-[#0d1f3c] rounded-xl px-6 py-6 sm:py-5 border border-white/[0.06] flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <a
            href="mailto:gameshopenter@gmail.com?subject=Inkoop%20aanvraag"
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-bold text-sm hover:from-emerald-600 hover:to-teal-600 transition-all shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/30 flex-shrink-0"
          >
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
            </svg>
      ...
      <AnimatePresence>
        {selected.size > 0 && (
          <motion.div
            ...
            className="fixed bottom-0 left-0 right-0 z-50 bg-white/95 dark:bg-slate-800/95 backdrop-blur-lg border-t border-emerald-200/80 dark:border-emerald-800/60 shadow-[0_-2px_16px_rgba(0,0,0,0.08)]"
          >
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="px-3.5 py-2 rounded-lg border border-slate-200 dark:border-slate-600 text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
                >
                  Wissen
                </button>
                <a
                  href={generateMailto()}
                  className="inline-flex items-center gap-2 px-5 py-2 rounded-lg bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-bold text-sm shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/35 transition-all"
                >
                  <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
                  </svg>
